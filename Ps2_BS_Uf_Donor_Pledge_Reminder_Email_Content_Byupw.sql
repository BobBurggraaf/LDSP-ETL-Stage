/****************************************************
	...
	Name: _Donor_Pledge_Reminder_Email_Content_Byupw_
	Date: 12/05/2018
...
GO
CREATE OR ALTER FUNCTION [dbo].[_Donor_Pledge_Reminder_Email_Content_Byupw_]()
	RETURNS TABLE
	AS 
	RETURN
	SELECT Donor_Key
		, CONCAT('<table width="100%"> <tr> <th align="left">Date</th> <th align="left">Amount</th> <th align="left">Fund Account</th> </tr> '
				...
				AS Donor_Pledge_Reminder_Email_Content_Byupw
		FROM
			(SELECT Donor_Key
							...
							FROM 
								(SELECT CONVERT(NVARCHAR(100),A.New_ConstituentDonor) AS Donor_Key
									...
									WHERE 1 = 1 
										...
										AND C.New_Inst = 'BYUPW' --Replace with respective institution
								) D
