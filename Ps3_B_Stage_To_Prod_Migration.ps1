					...
					IF(!(Test-Path $Log_File_Name))
						{
							...
							#---------------------------------------------	
							[STRING]$Src_Table_Qry = "SELECT Source_Table
																	FROM dbo.Stage_To_Prod
																	WHERE 1 = 1
																		AND Stage_To_Prod_Key = $Processing_Key
															  "                                                                                   
									$Src_Table = (Invoke-Sqlcmd -ServerInstance $Dest_Instance -Database $Dest_Db -Query $Src_Table_Qry).Source_Table
							...
							 
									...
									$Src_Table_Name = "[w16358\s01].LDSPhilanthropiesDW." + $Src_Table
